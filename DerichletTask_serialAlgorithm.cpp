...
////	U(x,0)=0 U(x,1)=x^2
////
//// Follow code implement serial algorithm
...
int main()
{
	...
	//	that will be consist of function values
	//	per each iterations
	...
	//
	// dmax - maximum deviation func value,
	//	must be within the error to end work cycle
	// it - count actual iteration
	double dmax;
	int it = 0;
	...
	{
		it++;
		// Set to zero deviation
		dmax = 0;
		for (int i = 1; i < N; ++i){
			for (int j = 1; j < N; ++j){
				double tmp = matrix[i][j];
				matrix[i][j] = 0.25*(matrix[i][j - 1] + matrix[i][j + 1] + matrix[i - 1][j] + matrix[i + 1][j]);
				double diff = fabs(tmp - matrix[i][j]);
				if (dmax < diff) 
					dmax = diff;
			}
		}
	// while deviation greater then required precision
	} while (dmax > e);
